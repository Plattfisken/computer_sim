...
//
void Clay_Raylib_Render(Clay_RenderCommandArray renderCommands, Font* fonts, UT_Arena *arena)
{
    for (int j = 0; j < renderCommands.length; j++)
    {
        ...
        switch (renderCommand->commandType)
        {
            case CLAY_RENDER_COMMAND_TYPE_TEXT: {
                ...
                char *temp_render_buffer = UT_arena_alloc(arena, strlen, 1);
                ...
                void *popped_address = UT_arena_pop(arena, strlen);
                assert(popped_address == temp_render_buffer);
